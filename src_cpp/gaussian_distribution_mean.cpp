...
int main ( int argc, char *argv[] )
{
    ...
    std::vector<std::string> random_noise_types{"uncorrelated_normal", "correlated_normal", "full_correlated_normal", "uncorrelated_student-t", "correlated_student-t", "full_correlated_student-t"};
    ...
    else if ( random_noise_type == random_noise_types[1] )
    {
        ...
        double offdiagonal_element = sigma_gaussion_distribution * sigma_gaussion_distribution * correlation;
        Eigen::VectorXd means = Eigen::VectorXd::Zero( dimension );
        sigma = Eigen::MatrixXd::Identity ( dimension, dimension ) *
                sigma_gaussion_distribution * sigma_gaussion_distribution;
        sigma.topRightCorner( dimension-1, dimension-1) += offdiagonal_element * Eigen::MatrixXd::Identity(dimension-1, dimension-1);
        sigma.bottomLeftCorner( dimension-1, dimension-1) += offdiagonal_element * Eigen::MatrixXd::Identity(dimension-1, dimension-1);
    ...
    else if (random_noise_type == random_noise_types[2])
    {
        ...
        double offdiagonal_element = sigma_gaussion_distribution * sigma_gaussion_distribution * correlation;
        Eigen::VectorXd means = Eigen::VectorXd::Zero( dimension );
        sigma = Eigen::MatrixXd::Constant ( dimension, dimension, offdiagonal_element );
        sigma += Eigen::MatrixXd::Identity(dimension, dimension) * (1 - correlation) * sigma_gaussion_distribution * sigma_gaussion_distribution;
    ...
    else if (random_noise_type == random_noise_types[3])
    {
        ...
        dataset = renyi_entropy::fromEigenMatrix<double, Eigen::MatrixXd>(dataset2);        
        renyi_entropy_function = [degrees_of_freedom](double q, Eigen::MatrixXd &sigma)
        {
    ...
    for ( const auto &item_alpha : calculator.GetAlphas() )
    {
        ...
        auto theoretical_renyi_entropy = renyi_entropy_function ( static_cast<double> ( item_alpha ), sigma );
        std::cout << theoretical_renyi_entropy << " ";
        for ( const auto &item_index : calculator.GetIndices() )
        {
            auto result_relative_entropy = theoretical_renyi_entropy / result[std::make_tuple ( item_index, item_alpha )];
