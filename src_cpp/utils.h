...
void convert_array ( const S &array, std::vector<T> &values,
                     T ( *convertor ) ( const std::string &, std::size_t * ) )
{
    try {
...
template <typename S, typename T>
void convert_array (
    const S &array, std::vector<T> &values,
    std::function<T ( const std::string &, std::size_t * ) > convertor )
{
    try {
        std::string_view data{array};
        std::string::size_type position{0};
...
template <typename S, typename T>
void chop_string_arrays ( const S &array, char chop_separator,
                          ...
                          T ( *convertor ) ( const std::string &, std::size_t * ) )
{
    ...
    for ( ; location_separator != array.npos; ) {
        ...
        convert_array<std::string_view, T> ( chop_substring, values.back(),
                                             convertor );
...
template <typename S, typename T>
void chop_string_arrays (
    const S &array, char chop_separator, std::vector<std::vector<T>> &values,
    std::function<T ( const std::string &, std::size_t * ) > convertor )
{
    size_t position{0};
    size_t location_separator{0};
