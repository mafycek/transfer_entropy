...
int main(int argc, char *argv[])
{
    ...
    unsigned int postselection_Y_history;
		...
		for (auto swap_datasets: {true, false})
		{
			...
			{
				auto [dataset1, dataset2] = renyi_entropy::renyi_entropy<double>::prepare_dataset ( dataset, swap_datasets, shuffle_dataset, 1, 1 );
				...
				for (auto & future_first: future_firsts)
				{
					for (auto & histories_first: history_firsts)
					{
						for (auto & histories_second: history_seconds)
						{
							...
							auto [y_future, y_history, z_history] = renyi_entropy::renyi_entropy<double>::PrepareDatasetForTransferEntropy( dataset1, dataset2, configuration_prepare_dataset );

							auto indices_to_use = std::ranges::iota_view{1U, maximal_neighborhood};
							std::map<std::string, std::any> configuration_renyi_entropy = {
								...
								std::pair <std::string, std::any>("indices_to_use", indices_to_use)
							};
