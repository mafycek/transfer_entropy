...
constexpr std::array<std::string, 4> RTE_type_extra_names{"effective", "nonlinear", "ballance", "reverse"};
constexpr std::array<std::string, 4> RTE_type_extra_abbreviation_names{"eff", "nonlin", "bal", "rev"};
...
typedef std::variant<double, long, unsigned int, std::string, std::vector<std::string>, std::array<std::string, 2>, std::array<unsigned int, 2>, std::vector<std::tuple<std::string, std::string> >, std::array<double, 2>, std::array<float, 2> > figure_parameter_t; //
typedef std::map<std::string, figure_parameter_t> parameter_t;
typedef std::set<std::tuple<bool, bool, bool, bool, bool, std::vector<unsigned int>, std::vector<unsigned int>, std::vector<unsigned int>>> selected_collection_keys_t;
...
{
    long number_of_columns_in_legend = 6L;
    convert_figure_parameters ( parameters, "cols", number_of_columns_in_legend );
    long dpi = 600L;
    ...
    std::string selected_type_of_calculation{renyi_entropy::conditional_renyi_entropy_label};
    convert_figure_parameters ( parameters, "type_of_calculation", selected_type_of_calculation );
    std::array<double, 2> xlim{0, 5};
    convert_figure_parameters ( parameters, "xlim", xlim );

    selected_collection_keys_t selected_collection_keys;
    ...
    {
        auto [type_of_calculation, ballance_dataset, effective_dataset, swap_datasets, shuffle_indicator, surrogate_indicator, neighbour, future_first, histories_first, histories_second, statistics, type_of_averaging] = item;
        if ( ( type_of_averaging == average_type_selector ) && ( type_of_calculation == selected_type_of_calculation ) )
        {
            selected_collection_keys.insert ( std::make_tuple ( ballance_dataset, effective_dataset, swap_datasets, shuffle_indicator, surrogate_indicator, future_first, histories_first, histories_second ) );
            std::cout << type_of_calculation << " " << statistics << " " << neighbour << " " << type_of_averaging << " " << swap_datasets << " " << shuffle_indicator << " " << surrogate_indicator << std::endl;
        }
    }
    ...
    for ( auto& suffix: suffices )
    {
        ...
        {
            auto [RTE_balance, RTE_effective, RTE_reverse_orientation, RTE_shuffled, RTE_surrogate, first_future, first_history, second_history] = column_item;
            ...
            const std::string RTE_type_extra = std::format ( "{}", std::string{"\\textrm{"} + ( RTE_balance ? RTE_type_extra_abbreviation_names[2]:"" ) + ( RTE_balance && ( RTE_shuffled||RTE_surrogate ) ? "," : "" ) + ( RTE_shuffled ? RTE_type_extra_abbreviation_names[0]:"" ) + ( ( RTE_shuffled && RTE_surrogate) ? "," : "" ) + ( RTE_surrogate ? RTE_type_extra_abbreviation_names[1]:"" )  + "}" );
            ...
            {
                auto collection_of_data = processed_results[ std::make_tuple ( selected_type_of_calculation, RTE_balance, RTE_effective, RTE_reverse_orientation, RTE_shuffled, RTE_surrogate, neighbour, first_future, first_history, second_history, selector, average_type_selector ) ];

                std::string *source_timeseries{&RTE_source_names[0]}, *destination_timeseries{&RTE_source_names[1]};
                if ( RTE_reverse_orientation )
                {
                    std::swap(source_timeseries, destination_timeseries);
                }
            ...
            py::dict kwargs = py::dict ( "loc"_a = py::str ( "best" ), "ncols"_a = py::int_ ( number_of_columns_in_legend ), "fontsize"_a = py::int_(4) /*py::str ( "xx-small" ) */ );
            if ( average_type_selector == renyi_entropy::average_runs )
            {
                plt.matplotlib_pyplot_legend ( **kwargs );
            }
            //plt::show();
            plt.matplotlib_pyplot_xlim(py::float_(xlim[0]), py::float_(xlim[1]));
...
void quantile_result_image ( renyi_entropy::renyi_entropy<calculation_type>::type_average_result_conditional_information_transfer_type & processed_results, parameter_t & parameters )
{
    ...
    for ( const auto & item: collection_keys )
    {
        auto [type_of_calculation, balance_dataset, effective_dataset, swap_datasets, shuffle_indicator, surrogate_indicator, neighbour, future_first, histories_first, histories_second, statistics, type_of_averaging] = item;
        if ( ( type_of_averaging == average_type_selector ) && ( type_of_calculation == selected_type_of_calculation ) )
        {
            selected_collection_keys.insert ( std::make_tuple ( balance_dataset, effective_dataset, swap_datasets, shuffle_indicator, surrogate_indicator, future_first, histories_first, histories_second ) );
            std::cout << type_of_calculation << " " << statistics << " " << neighbour << " " << type_of_averaging << std::endl;
        }
    }
    ...
    for ( const auto& suffix: suffices )
    {
        ...
        for ( const auto & column_item : selected_collection_keys )
        {
            ...
            const auto file_output_name{prefix_filename + output_base_name + "_" + filename_history_suffix + "." + suffix};
            std::cout << file_output_name << std::endl;
            const std::string RTE_type_extra = std::format ( "{}", std::string{"\\textrm{"} + ( RTE_balance ? RTE_type_extra_abbreviation_names[2]:"" ) + ( RTE_shuffled ? RTE_type_extra_abbreviation_names[0]:"" ) + ( RTE_shuffled && RTE_surrogate ? "," : "" ) + ( RTE_surrogate ? RTE_type_extra_abbreviation_names[1]:"" )  + "}" );
            const auto RTE_type = "{" + std::format ( "\\left( R, {} \\right)", RTE_type_extra ) + "}";
            ...
            for ( auto neighbour = limits[0]; neighbour <= limits[1] ; ++ neighbour )
            {
                ...
                {
                    auto upper_collection_of_data = processed_results[ std::make_tuple ( selected_type_of_calculation, RTE_balance, RTE_effective, RTE_reverse_orientation, RTE_shuffled, RTE_surrogate, neighbour, first_future, first_history, second_history, std::get<0> ( selector_fill ), average_type_selector ) ];
                    auto lower_collection_of_data = processed_results[ std::make_tuple ( selected_type_of_calculation, RTE_balance, RTE_effective, RTE_reverse_orientation, RTE_shuffled, RTE_surrogate, neighbour, first_future, first_history, second_history, std::get<1> ( selector_fill ), average_type_selector ) ];
            ...
            py::dict kwargs = py::dict ( "loc"_a = py::str ( "best" ), "ncols"_a = py::int_ ( number_of_columns_in_legend ), "fontsize"_a = py::str ( "xx-small" ) );
            if ( average_type_selector == renyi_entropy::average_runs )
            {
                plt.matplotlib_pyplot_legend ( **kwargs );
            }
...
void deviation_result_image ( renyi_entropy::renyi_entropy<calculation_type>::type_average_result_conditional_information_transfer_type & processed_results, parameter_t & parameters )
{
    ...
    for ( const auto & item: collection_keys )
    {
        ...
        {
            selected_collection_keys.insert ( std::make_tuple ( balance_dataset, effective_dataset, swap_datasets, shuffle_indicator, surrogate_indicator, future_first, histories_first, histories_second ) );
            std::cout << std::format ( "{} {} {} {} {} {} {} {}", balance_dataset, effective_dataset, swap_datasets, shuffle_indicator, surrogate_indicator, neighbour, statistics, type_of_averaging ) << std::endl;
        }
    }
    ...
    for ( const auto& suffix: suffices )
    {
        ...
        for ( const auto & column_item : selected_collection_keys )
        {
            ...
            for ( auto neighbour = limits[0]; neighbour <= limits[1] ; ++ neighbour )
            {
                ...
                for ( const auto &[raw_color_index, selector_fill] : std::views::enumerate ( selector_fills ) )
                {
                    auto mean_dataset = processed_results[ std::make_tuple ( selected_type_of_calculation, RTE_balance, RTE_effective, RTE_reverse_orientation, RTE_shuffled, RTE_surrogate, neighbour, first_future, first_history, second_history, std::get<0> ( selector_fill ), average_type_selector ) ];
                    auto std_dataset = processed_results[ std::make_tuple ( selected_type_of_calculation, RTE_balance, RTE_effective, RTE_reverse_orientation, RTE_shuffled, RTE_surrogate, neighbour, first_future, first_history, second_history, std::get<1> ( selector_fill ), average_type_selector ) ];
            ...
            for ( auto neighbour = limits[0]; neighbour <= limits[1] ; ++ neighbour )
            {
                ...
                {
                    auto mean_dataset = processed_results[ std::make_tuple ( selected_type_of_calculation, RTE_balance, RTE_effective, RTE_reverse_orientation, RTE_shuffled, RTE_surrogate, neighbour, first_future, first_history, second_history, ( selector_fill ), average_type_selector ) ];
                    ...
                    const auto color_position_within_colormap = static_cast<double> ( raw_color_index + 1 ) / ( static_cast<double> ( selector_fills.size() ) != 1 ? static_cast<double> ( selector_fills.size() - 1 ) : 1 );
                    auto color = colormap ( color_position_within_colormap );
                    {
                        py::dict kwargs = py::dict ( "color"_a = color );
                        if ( average_type_selector == renyi_entropy::average_runs )
...
int main ( int argc, char *argv[] )
{
    ...
    ( "file,f", po::value<std::string>()->default_value ( "CRE_processed.bin.zstd" ), "Output file" );
    ...
    CPPTRACE_TRY
    {
        ...
        std::string input = vm["file"].as<std::string>();
        ...
        if ( input_file_handler.is_open() )
        {
            ...
            auto &processed_results = result_conditional_information_transfer;

            auto maximal_neighbor = 100U;
            {
                parameter_t parameters_fill{{"output_base_name", "H1"}, {"dpi", 600}, {"color_map", "plasma"}, {"average_type_selector", renyi_entropy::average_runs}, {"selector", "mean"}, {"color_map_selector", "jet"}, {"alpha", 0.8}, {"limits", std::array<unsigned int, 2>{2, maximal_neighbor}}, {"type_of_calculation", renyi_entropy::renyi_entropy_X_present_history_label}};
                simple_result_image ( processed_results, parameters_fill );
            }
            {
                parameter_t parameters_fill{{"output_base_name", "H2"}, {"color_map", "plasma"}, {"average_type_selector", renyi_entropy::average_runs}, {"selector", "mean"}, {"color_map_selector", "jet"}, {"alpha", 0.8}, {"limits", std::array<unsigned int, 2>{2, maximal_neighbor}}, {"type_of_calculation", renyi_entropy::renyi_entropy_X_history_label}};
                simple_result_image ( processed_results, parameters_fill );
            }
            {
                parameter_t parameters_fill{{"output_base_name", "H3"}, {"color_map", "plasma"}, {"average_type_selector", renyi_entropy::average_runs}, {"selector", "mean"}, {"color_map_selector", "jet"}, {"alpha", 0.8}, {"limits", std::array<unsigned int, 2>{2, maximal_neighbor}}, {"type_of_calculation", renyi_entropy::joint_renyi_entropy_label}};
                simple_result_image ( processed_results, parameters_fill );
            }
            {
                parameter_t parameters_fill{{"output_base_name", "H4"}, {"color_map", "plasma"}, {"average_type_selector", renyi_entropy::average_runs}, {"selector", "mean"}, {"color_map_selector", "jet"}, {"alpha", 0.8}, {"limits", std::array<unsigned int, 2>{2, maximal_neighbor}}, {"type_of_calculation", renyi_entropy::renyi_entropy_XY_history_label}};
                simple_result_image ( processed_results, parameters_fill );
            }

            {
                parameter_t parameters_fill{{"output_base_name", "std_RTE"}, {"color_map", "plasma"}, {"average_type_selector", renyi_entropy::average_runs_neighbors}, {"selector_fills", std::vector<std::tuple<std::string, std::string>>{std::tuple<std::string, std::string>{"mean", "standard_deviation"}}}, {"selector", "mean"}, {"color_map_selector", "jet"}, {"alpha", 0.8}, {"limits", std::array<unsigned int, 2>{0, 1}}, {"type_of_calculation", renyi_entropy::conditional_renyi_entropy_label}};
                deviation_result_image ( processed_results, parameters_fill );
            }

            {
                parameter_t parameters { {"selector", "mean"}, {"color_map", "plasma"}, {"output_base_name", "mean_RTE_R"}, {"average_type_selector", renyi_entropy::average_runs}, {"alpha", 0.8}, {"limits", std::array<unsigned int, 2>{5, maximal_neighbor}} };
            ...
            {
                parameter_t parameters{ {"selector", "mean"}, {"color_map", "plasma"}, {"output_base_name", "mean_RTE"}, {"average_type_selector", renyi_entropy::average_runs_neighbors}, {"alpha", 0.8}, {"limits", std::array<unsigned int, 2>{0, 0}} };
                simple_result_image ( processed_results, parameters );
