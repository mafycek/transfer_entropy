...
def complete_test_1D(
        ...
        mu=0,
        sigmas=(0.1, 0.5, 1.0, 5.0, 10.0, 50, 100),
        ...
        sample_estimator=lambda data_samples, alpha: transfer_entropy.renyi_entropy(
            data_samples, method="LeonenkoProzanto", indices_to_use=[1, 2, 3, 4, 5], alpha=alpha
        ),
):
...
def complete_test_ND(
        filename="statistics.txt",
        samples=1000,
        sigma_skeleton=np.identity(10),
        alphas=(
                0.1,
                0.2,
                0.3,
        ...
        sigmas=(0.1, 0.5, 1.0, 5.0, 10.0, 50, 100),
        sizes_of_sample=(10, 20, 50, 100, 200, 500, 1000, 2000, 5000, 10000, 20000, 50000),
        indices_to_use=(1, 2, 3, 4, 5, 6, 7, 8, 9),
        theoretical_value_function=lambda sigma, alpha, determinant: Renyi_normal_distribution_ND(
            sigma, alpha, determinant
        ),
        sample_generator=lambda mu, sigma, size_sample: sample_normal_distribution(sigma, size_sample),
        sample_estimator=lambda data_samples, alpha, indices_to_use: transfer_entropy.renyi_entropy(
            data_samples, method="LeonenkoProzanto", indices_to_use=indices_to_use, alpha=alpha
        ),
        determinant=None,
        **kwargs,
):
    ...
    with open(filename, "wt") as fd:
        ...
        print(
            "mean Renyi entropy\tstd Renyi entropy\tmean difference\tstd of difference\t3rd moment of difference\t4th moment of difference",
            file=fd,
        )
        ...
        for sigma in sigmas:
            ...
            for size_sample in sizes_of_sample:
                for sample in range(1, samples + 1):
                    if sample % 10 == 0:
                        print(
                            f"size_sample = {size_sample}, sigma={sigma}, sample = {sample}, indices_to_use={indices_to_use}"
                        )
                    ...
                    for alpha in alphas:
                        ...
                        entropy_list = entropy[alpha]
                        difference[alpha] = [entropy_item - theoretical_value for entropy_item in entropy_list]
                ...
                for alpha in alphas:
                    ...
                    for index, index_used in enumerate(indices_to_use):
                        ...
                        difference_sample = [
                            sample[alpha][index] for sample_number, sample in difference_samples.items()
                        ]
                    ...
                    entropy_samples_all = [
                        item for sample_number, sample in entropy_samples.items() for item in sample[alpha]
                    ]
                    difference_samples_all = [
                        item for sample_number, sample in difference_samples.items() for item in sample[alpha]
                    ]
...
def small_test():
    ...
    for alpha in alphas:
        for number_samples in [10, 20, 50, 100, 200, 500, 1000, 2000, 5000, 10000, 20000, 50000, 100000]:
            ...
            time_start = time.process_time()
            entropy = mutual_inf.renyi_entropy(
                data_samples, method="LeonenkoProzanto", indices_to_use=[1, 2, 3, 4, 5], alpha=alpha
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument("--output", metavar="XXX", type=str, default="complete_statistics", help="Base filename")
    parser.add_argument("--dimensions", metavar="XXX", type=int, nargs="+", help="Dimensions")
    parser.add_argument(
        "--method",
        metavar="XXX",
        type=str,
        default="LeonenkoProzanto",
        help='Method of Renyi calculation: ["Paly", "LeonenkoProzanto"]',
    )
    ...
    parser.add_argument("--correlation", metavar="XXX", type=float, default=[0], nargs="+", help="Correlation strength")
    parser.add_argument(
        "--correlation_type",
        metavar="XXX",
        type=str,
    ...
    parser.add_argument(
        ...
        type=str,
        default="gaussian",
        help='Noise type: ["gaussian", "student", "sub_gaussian", "laplace"]',
    )
    parser.add_argument(
        "--arbitrary_precision", metavar="XXX", type=str, default="False", help="Use of the arbitrary precision"
    )
    parser.add_argument(
        ...
        type=int,
        default="30",
        help="How many decimal numbers are used",
    )
    parser.add_argument("--metric", metavar="XXX", type=str, default="euclidean", help="Metric")
    parser.add_argument("--degrees_of_freedom", metavar="XXX", type=float, default=2 - 0.0, help="Degrees of freedom")
    ...
    job_dictionary = {
        "gaussian": {
            "generator": lambda mu, sigma, size_sample: sample_normal_distribution(sigma, size_sample),
            "theory": lambda sigma, alpha, determinant: Renyi_normal_distribution_ND(sigma, alpha, determinant),
        },
        "student": {
            "generator": lambda mu, sigma, size_sample: sample_student_t_distribution(
                degrees_of_freedom, sigma, mu, size_sample
            ),
            "theory": lambda sigma, alpha, determinant: Renyi_student_t_distribution(
                degrees_of_freedom, sigma, alpha, determinant
            ),
        },
        "sub_gaussian": {
            "generator": lambda mu, sigma, size_sample: sample_elliptical_contour_stable(sigma, alpha, mu, size_sample),
            "theory": None,
        },
        "laplace": {
            "generator": lambda mu, sigma, size_sample: sample_asymmetric_laplace_distribution(sigma, mu, size_sample),
            "theory": None,
        },
    }
    estimator_dictionary = {
        "Renyi": lambda data_samples, alpha, indices_to_use: transfer_entropy.renyi_entropy(
            data_samples,
            method=method,
        ...
        "Renyi_float": lambda data_samples, alpha, indices_to_use: transfer_entropy.renyi_entropy(
            data_samples,
            method=method,
            indices_to_use=indices_to_use,
            alphas=alpha,
    ...
    for dimension in dimensions:
        ...
        for correlation in correlations:
            ...
            complete_test_ND(
                filename=f"{output_filename}_{method}_{correlation}_{dimension}.txt",
                samples=3,
                sigmas=sigmas,
                alphas=alphas,
                sigma_skeleton=sigma_skeleton,
                sizes_of_sample=samples_sizes,
