...
from mathematical_models.random_samples import *
        ...
        try:
            if index_column % 20 == 0:
                ax.plot(
                    ...
                    linewidth=1,
                    linestyle="dotted",
                    label=f"Order {index_column}",
                )
            else:
                ax.plot(
                    xs.values, ys.values, color=color, linewidth=1, linestyle="dotted"
                )
    ...
    for comparison_column in comparison_columns:
        ...
        try:
            if "error_column" in comparison_column:
                ...
                ax.errorbar(
                    xs.values, ys.values, yerr=errorbar.values, color=color, label=label
                )
            else:
...
if __name__ == "__main__":
    ...
    job_dictionary = {
        "gaussian": {
            "theory": lambda sigma, alpha, determinant: Renyi_normal_distribution_ND(
    ...
    def line_processing(line):
        ...
        prediction = job_dictionary["gaussian"]["theory"](
            output[0], line["alpha"], output[1]
        )
    ...
    set_of_theoretical_values = [
        {"column": "theoretical value", "color": "b", "label": "Theoretical value"},
        {
            "column": "mean Renyi entropy",
            "color": "r",
            "label": r"Mean of order 95\% bars",
            "error_column": "std Renyi entropy",
        },
    ]
    set_of_difference_values = [
        {
            "column": "mean difference",
            "color": "r",
            "label": r"Mean of order 95\% bars",
            "error_column": "std of difference",
        }
    ]
    ...
    filtered_results = filter_by(
        results, ["correlation", "sigma", "dimension", "sample size"]
    )
    ...
    for key, filtered_results in filtered_results.items():
        ...
        figure(
            ...
            "alpha",
            set_of_theoretical_values,
            sets_of_mean_results,
            r"\Large Comparison of calculated Renyi entropy and theoretical value",
            r"$\alpha$",
            r"$H_{\alpha}(X)$",
            f"{data_directory}/Entropy_comparison_correlation_{correlation}_sigma_{sigma}_dimension_{dimension}_samplesize_{sample_size}",
            suffix="eps",
        )
        figure(
            filtered_results,
            "alpha",
            set_of_difference_values,
            sets_of_mean_difference_results,
            r"\Large Differences from theoretical value",
            r"$\alpha$",
            r"$H^{est}_{\alpha}(X)-H^{theor}_{\alpha}(X)$",
            f"{data_directory}/differences_comparison_correlation_{correlation}_sigma_{sigma}_dimension_{dimension}_samplesize_{sample_size}",
            suffix="eps",
        )
