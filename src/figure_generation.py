...
def generate_figures_from_data(TE, TE_cathegories, TE_cathegories_std, TE_nonswapped_columns, symbols, directory,
                               name_of_title, figure_parameters: typing.Dict):
    for symbol in symbols:
        ...
        for item in []:  # TE_nonswapped_columns:
            try:
                ...
                label_latex_std = figure_parameters["latex_label_size"] + f"""$\\sigma_{{{label}}}$"""
...
def generate_figures_from_neighborhood_statistics(TE_statistics, figure_parameters: typing.Dict):
    selection = TE_statistics.xs("Avaraging over sample for each neighbour k", level='Remark', axis=1)
    ...
    for item in selection.columns:
        RTE_type, history_first, future_first, history_second, type_of_statistics, shuffled_dataset, reversed_order, sample_number, timeserie_name = item
    ...
    subList = list(filter(lambda elem: "quantile" in elem or "median" in elem, statistics_types))
    sublist = [
        {"selector": ["quantile 0.1", "quantile 0.9"], "color": "moccasin", "style": "quantile band",
         "label": "0.1-0.9 quantile band", "aggregation": "sample"},
        {"selector": ["quantile 0.25", "quantile 0.75"], "color": "lightcoral", "style": "quantile band",
         "label": "0.25-0.75 quantile band", "aggregation": "sample"},
        {"selector": ["median"], "color": "greenyellow", "style": "line", "label": "median", "aggregation": "sample"}
    ]
    sublist_std = [
        {"selector": ["mean", "std"], "color": "moccasin", "style": "yerrorbar", "label": "errorbar",
         "aggregation": "sample"},
        {"selector": ["mean"], "color": "greenyellow", "style": "line", "label": "mean", "aggregation": "sample"}
    ]
    ...
    for timeserie_name in list(timeserie_names):
        ...
        for RTE_type in RTE_types:
            RTE_type_selection = timeserie_selection.xs(RTE_type, level='Name of variable', axis=1)
            for time_set in list(time_sets):
                reverse_directions = [True, False] if "balance" not in RTE_type else [False]
                for actual_reverse_direction in reverse_directions:
                    shuffled_calculations = [True] if "effective" not in RTE_type else [False]
                    for actual_shuffled_calculation in shuffled_calculations:
                        history_first, future_first, history_second = time_set
                        time_set_selection_hf = RTE_type_selection.xs(history_first, level='History first', axis=1)
                        time_set_selection_ff = time_set_selection_hf.xs(future_first, level='Future first', axis=1)
                        time_set_selection_hs = time_set_selection_ff.xs(history_second, level='History second', axis=1)
                        time_set_selection_reverse = time_set_selection_hs.xs(actual_reverse_direction,
                                                                              level='Reversed direction', axis=1)
                        time_set_selection_complet = time_set_selection_reverse.xs(actual_shuffled_calculation,
                                                                                   level='Shuffled', axis=1)
                        ...
                        pure_title = name_of_title.capitalize().replace("_", " ") + f" of {symbol} for nearest neighbor"
                        latex_title = figure_parameters["latex_title_size"] + f"""{{{pure_title}}}"""
                        latex_alpha_label = figure_parameters["latex_overview_label_size"] + r"$\alpha$"
                        pure_title_std = "Standard deviation of " + name_of_title.capitalize().replace("_",
                                                                                                       " ") + f" of {symbol} for nearest neighbor"
                        latex_title_std = figure_parameters["latex_title_size"] + f"""{{{pure_title_std}}}"""
                        ...
                        standard_filename = (
                                ...
                                ("_shuffled" if shuffled_calculation else "") + "_quantiles_SA")
                        standard_filename_std = (
                                figure_parameters["directory"]
                                + f"/{base_filename}_{symbol}_{history_first}_{future_first}_{history_second}"
                                + ("_reversed" if actual_reverse_direction else "")
                                + ("_shuffled" if shuffled_calculation else "") + "_std_SA")
...
def generate_overview_figures_from_averages(TE_statistics, figure_parameters):
    selection = TE_statistics.xs("Avaraging over nearest neighbours starting from 5", level='Remark', axis=1)
    ...
    subList = list(filter(lambda elem: "quantile" in elem or "median" in elem, statistics_types))
    sublist = [
        {"selector": ["0.1 quantile", "0.9 quantile"], "color": "moccasin", "style": "quantile band",
         "label": "0.1-0.9 quantile band", "aggregation": "sample"},
        {"selector": ["0.25 quantile", "0.75 quantile"], "color": "lightcoral", "style": "quantile band",
         "label": "0.25-0.75 quantile band", "aggregation": "sample"},
        {"selector": ["median"], "color": "greenyellow", "style": "line", "label": "median", "aggregation": "sample"}
    ]
    sublist_std = [
        {"selector": ["mean", "std"], "color": "moccasin", "style": "yerrorbar", "label": "errorbar",
         "aggregation": "sample"},
        {"selector": ["mean"], "color": "green", "style": "line", "label": "mean", "aggregation": "sample"}
    ]

    print(list(timeserie_names), list(statistics_types), list(RTE_types), list(time_sets))

    for timeserie_name in list(timeserie_names):
        timeserie_selection = selection.xs(timeserie_name, level='Sample', axis=1)

        for RTE_type in RTE_types:
            RTE_type_selection = timeserie_selection.xs(RTE_type,
                                                        level='Name of variable', axis=1)
            for time_set in list(time_sets):
                reverse_directions = [True, False] if "balance" not in RTE_type else [False]
                for actual_reverse_direction in reverse_directions:
                    shuffled_calculations = [False, True] if "effective" not in RTE_type else [False]
                    for actual_shuffled_calculation in shuffled_calculations:
                        history_first, future_first, history_second = time_set
                        time_set_selection_hf = RTE_type_selection.xs(history_first, level='History first', axis=1)
                        time_set_selection_ff = time_set_selection_hf.xs(future_first, level='Future first', axis=1)
                        ...
                        rows = (len(samples) if actual_shuffled_calculation else 1) if isinstance(
                            time_set_selection_complet.iloc[0].iloc[0], float) else \
                        time_set_selection_complet.iloc[0].iloc[0].shape[0]

                        standard_filename = (
                                figure_parameters["directory"] +
                                f"/{base_filename}_{symbol}_{history_first}_{future_first}_{history_second}" +
                                ("_reversed" if actual_reverse_direction else "") +
                                ("_shuffled" if actual_shuffled_calculation else "") + "_quantiles" + "_NN")
                        standard_filename_std = (
                                figure_parameters["directory"]
                                + f"/{base_filename}_{symbol}_{history_first}_{future_first}_{history_second}"
                                + ("_reversed" if actual_reverse_direction else "")
                                + ("_shuffled" if actual_shuffled_calculation else "") + "_std" + "_NN")
