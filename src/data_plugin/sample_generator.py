...
def samples_from_arrays(data, **kwargs):
    ...
    sampled_dataset = np.zeros(
        (shape_of_array[0] * allocated_space, length_of_timeserie)
    )  # history
...
def preparation_dataset_for_transfer_entropy(
    ...
    marginal_solution_1_selected = (
        ...
        if skip_last == 0
        else marginal_solution_1[:, skip_last:-skip_last]
    )
    marginal_solution_2_selected = (
        marginal_solution_2[:, skip_last:]
        if skip_last == 0
        else marginal_solution_2[:, skip_last:-skip_last]
    )
    ...
    kwargs["skip_first"] = (
        time_shift_between_X_Y
        if history_x > history_y
        else history_y - history_x + time_shift_between_X_Y
    )
    if "history_index_x" in kwargs:
        if "future_index_x" in kwargs:
        ...
        kwargs["skip_first"] = max(history_index_y + history_index_x) + max(
            future_index_x
        )
        kwargs["history"] = history_x
...
if __name__ == "__main__":
    ...
    if test_sample in ["sample_array"]:
        ...
        solution = preparation_dataset_for_transfer_entropy(
            X_solution, Y_solution, **kwargs
        )
