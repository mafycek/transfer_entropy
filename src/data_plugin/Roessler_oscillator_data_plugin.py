...
class RoesslerOscillatorDataPlugin(GenericFilePlugin):
    ...
    def load_datasets(self):
        dataset = []
        try:
            with open(self.file, "rt") as file_handler:
                while True:
                    parameters = self.read_header(file_handler)
                    frame = self.read_dataset(file_handler, parameters)
    ...
    def prepare_dataset(
        ...
        if not args.dataset:
            ...
            if args.interpolate:
                number = int(
                    (args.t_stop - args.skip) * args.interpolate_samples_per_unit_time
                )
                ...
                for dimension in range(sol.y.shape[0]):
                    function = scipy.interpolate.interp1d(
                        sol.t, sol.y[dimension], kind="cubic"
                    )
...
if __name__ == "__main__":
    datasetPlugin = RoesslerOscillatorDataPlugin()
    dataset = datasetPlugin.load_datasets()
    print(f"We aggregated {len(dataset)} records")
    datasetPlugin.pickle_dataset(dataset)
