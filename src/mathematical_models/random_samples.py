...
def Renyi_laplace_assymetric_distribution(sigma, mean, alpha, sigma_inversion=None):
    ...
    prefactor = (
            pow(2 * np.pi, dimension)
            / (2 * pow(alpha, dimension))
            * gamma(k + 1)
            / gamma(alpha)
            * math.exp(one_over_2M)
            * pow(2 / M, k + 1)
            * Fox
            - H
    )
...
def sample_normal_distribution(sigma, size_sample=10):
    if (
            ...
            and len(sigma.shape) == 2
            and (sigma.shape[0] == sigma.shape[1])
    ):
        dimension = sigma.shape[0]
...
def sample_elliptical_contour_stable(
        sigma, alpha, gamma=1.0, delta=np.array([0]), size_sample=10
):
    recalculated_gamma = (
            2 * np.power(gamma, 2) * np.power(np.cos(np.pi * alpha / 4), (2 / alpha))
    )
    random_stable = stat.levy_stable.rvs(
        alpha=alpha / 2.0, beta=1.0, loc=0, scale=recalculated_gamma, size=size_sample
    )
    ...
    stable_samples = np.array(
        [
            multiplicator * vector - delta
            for multiplicator, vector in zip(sqrt_random_stable, random_normal)
        ]
    )
...
def sample_sub_linear_levy_stable(
        sigma, alpha, alpha_orig, gamma=1.0, delta=np.array([0]), size_sample=10
):
    pass


def sample_linear_depended_stable(
        sigma, alpha, beta=np.array([0]), delta=np.array([0]), size_sample=10
):
    dimension = sigma.shape[0]
    if (dimension == beta.shape[0]) and (dimension == delta.shape[0]):
        ...
        stable_samples = np.array(
            [np.dot(sigma, vector) for vector in random_stable_sample]
        )
...
def sample_asymmetric_laplace_distribution(
        sigma=np.array([1]), mean=np.array([0]), size_sample=10
):
    ...
    stable_samples = np.array(
        [
            mean * multiplicator + multiplicator_sqrt * vector
            for multiplicator, multiplicator_sqrt, vector in zip(
            exponentially_distributed, sqrt_exponentially_distributed, random_normal
        )
        ]
    )
...
def sample_student_t_distribution(
        degrees_of_freedom, sigma=np.array([1]), mean=np.array([0]), size_sample=10
):
    ...
    stable_samples = np.array(
        [
            mean + vector * (np.sqrt(degrees_of_freedom / multiplicator))
            for multiplicator, vector in zip(chi_squared_distributed, random_normal)
        ]
    )
...
def Renyi_student_t_distribution(
        degrees_of_freedom: float, sigma, q: float, determinant=None
):
    dimension = sigma.shape[0]
    ...
    if determinant:
        arg = pow(determinant, (1.0 - q) / 2)
    else:
        arg = pow(np.linalg.det(sigma), (1.0 - q) / 2)

    arg2 = pow(degrees_of_freedom * np.pi, dimension / 2 * (1.0 - q))
    ...
    if (
            isinstance(arg3, mpmath.mpf)
            or isinstance(arg, mpmath.mpf)
            or isinstance(arg2, mpmath.mpf)
    ):
...
def Renyi_normal_distribution_1D(sigma_number, alpha):
    if alpha == 1:
        ...
        return (
                ...
                + math.log(sigma_number)
                + math.log(alpha) / (alpha - 1) / 2
        )
...
def Renyi_normal_distribution_ND(sigma_matrix: np.matrix, alpha, determinant=None):
    ...
    if alpha == 1:
        return math.log(2 * math.pi * math.exp(1)) * dimension / 2.0 + np.log(
            np.sqrt(np.linalg.det(sigma_matrix))
        )
    else:
        if determinant:
            return (
                    math.log(2 * math.pi) * dimension / 2.0
                    + math.log(determinant) / 2.0
                    + dimension * math.log(alpha) / (alpha - 1) / 2
            )
        else:
            return (
                    math.log(2 * math.pi) * dimension / 2.0
                    + math.log(np.linalg.det(sigma_matrix)) / 2.0
                    + dimension * math.log(alpha) / (alpha - 1) / 2
            )
...
def Renyi_student_t_distribution_1D(sigma, degrees_of_freedom, alpha):
    ...
    if alpha == 1:
        return (degrees_of_freedom + 1.0) / 2 * (
                spec.digamma((degrees_of_freedom + 1.0) / 2)
                - spec.digamma((degrees_of_freedom) / 2)
        ) + np.log2(
        ...
        nominator = spec.beta(
            dimension / 2.0,
            alpha * (dimension + degrees_of_freedom) / 2.0 - dimension / 2.0,
        )
        denominator = math.pow(
            spec.beta(degrees_of_freedom / 2.0, dimension / 2.0), alpha
        )
        beta_factor = math.log2(nominator / denominator)
        return 1 / (1 - alpha) * beta_factor * math.log2(
            math.pow(np.pi * degrees_of_freedom, dimension) * determinant
        ) - math.log2(spec.gamma(dimension / 2.0))
...
def Renyi_beta_distribution(a, b, alpha):
    return (
            ...
            / (1 - alpha)
            * math.log2(
        spec.beta(alpha * a + alpha - 1, alpha * b + alpha - 1)
        / math.pow(spec.beta(a, b), alpha)
    )
    )
...
if __name__ == "__main__":
    ...
    stable_linear_depended_samples = sample_linear_depended_stable(
        sigma=sigma,
        alpha=1.999,
    ...
    samples_stable = sample_elliptical_contour_stable(
        sigma, 1.95, 1.0, np.array([0, 0]), size_sample=samples
    )
    samples_laplace = sample_asymmetric_laplace_distribution(
        sigma=sigma, mean=np.array([1, -1]), size_sample=samples
    )
    samples_student = sample_student_t_distribution(
        sigma=sigma, mean=np.array([0, 0]), degrees_of_freedom=2, size_sample=samples
    )

    datasamples = [
        [normal_samples, "Normal distribution"],
        [stable_linear_depended_samples, "Stable linear dependent"],
        [samples_stable, "Stable sub-Gaussian"],
        [samples_laplace, "Laplace asymmetric"],
        [samples_student, "Student t-distribution"],
    ]
    ...
    for index, (sample, title) in enumerate(datasamples):
        ...
        ax2.hist2d(
            sample[:, 0], sample[:, 1], bins=100, norm=mcolors.LogNorm(), cmap="Reds"
        )
        plt.ion()
        plt.show()
        plt.close()
