...
def roessler_oscillator(**kwargs):
    ...
    if not cache or not os.path.isfile(path):
        ...
        solution = solve_ivp(
            lambda t, y: right_side_ivp(t, y, params),
            [tStart, tStop],
            y0,
            method=method,
        )
...
def roessler_3d_plot(dataset, configurations, title, filename, suffix, dpi=300):
    ...
    fig.suptitle(title)
    plt.subplots_adjust(
        hspace=0.24, wspace=0.24, left=0.05, right=0.95, top=0.95, bottom=0.05
    )
...
if __name__ == "__main__":
    dataset = False
    if dataset:
        from src.data_plugin.Roessler_oscillator_data_plugin import load_static_dataset
        ...
        for dataset_item in datasets:
            ...
            roessler_plot(
                dataset,
                r"Evolution of Rössler oscilator $\varepsilon=$",
                "roessler-" + str(metadata["eps1"]),
                "pdf",
            )
    else:
        configuration = {
        ...
        for epsilon in [-0.34, -0.35, -0.4]:  # np.arange(0.0, 0.241, 0.005)
            ...
            roessler_3d_plot(
                sol.y,
                configuration,
                rf"\Large Evolution of Rössler oscilator $\varepsilon= {epsilon}$",
                f"roessler-{epsilon}",
                "png",
            )
