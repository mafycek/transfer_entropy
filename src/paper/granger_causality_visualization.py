...
def granger_function(alpha, k, l):
    ...
    xi = (1.0 / (1.0 - alpha)) - (k / 2.0)
    dzeta = (1.0 / (1.0 - alpha)) - ((k + l) / 2.0)
    if numpy.isinf(dzeta) or numpy.isinf(xi):
        print(alpha, k, l)
    return numpy.log2(
        numpy.power(
            math.e, special.gammaln(xi - 0.5) - special.gammaln(xi)
        )  # math.gamma(xi - 0.5) / math.gamma(xi)
        * numpy.power(
            math.e, xi * numpy.log(xi - 1) - (xi - 0.5) * numpy.log(xi - 1.5)
        )  # * numpy.power(xi - 1, xi) / numpy.power(xi - 1.5, xi - 0.5)
        * math.gamma(dzeta)
        / math.gamma(dzeta - 0.5)
        * numpy.power(
            math.e,
            (dzeta - 0.5) * numpy.log(dzeta - 1.5) - dzeta * numpy.log(dzeta - 1),
        )
...
if __name__ == "__main__":
    ...
    for k in numpy.arange(1, 11, 0.01):
        ...
        n = 500.0 + 1
        limit_alpha = 0.989
        difference = ((3.0 + k) / (5.0 + k) - limit_alpha) / n
        for alpha in numpy.arange(limit_alpha, (3.0 + k) / (5.0 + k), difference):
            try:
